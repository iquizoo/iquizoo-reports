...
scores_combined_this_ability %>%
  ...
  geom_vline(xintercept = mean(scores_unit$score, na.rm = TRUE), color = "red") +
...
本校学生总体的`r ab_name_cn`测评的得分分布见图\@ref(fig:<<ab_id>>-histogram)。本校学生的整体表现`r diff_msg`。
...
本校学生总体在`r ab_name_cn`测评中各个水平的人数比例如图\@ref(fig:<<ab_id>>-grading)。每个班级的学生从等级优秀到不足的占比情况存在差异，应该重点关注在等级D中占比较大的班级对学生的`r ab_name_cn`进行有计划的训练和干预。按照学生总体表现划分ABCD四个水平：其中，**水平A**为优秀，代表总体水平的90%及以上；**水平B**为良好，代表总体水平的70%至90%；**水平C**为中等，代表总体水平的30%至70%；**水平D**为较差，代表总体水平的0%至30%。

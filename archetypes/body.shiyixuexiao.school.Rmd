...
```{r {{ab_code}}-histogram, fig.cap=str_glue("{{name_unit}}总体学生{{ab_name}}测评的分数分布"), fig.width=8, fig.asp=0.618}
...
```{r {{ab_code}}-grading, include="{{ab_code}}"=="blai", fig.width=8, fig.height=1, fig.cap="评分等级划分"}
...
```{r {{ab_code}}-boxplot-class, fig.cap=str_glue("本次测评全体、本校及各班级学生{{ab_name}}测评得分情况对比"), fig.width=8, fig.asp=1.2}
list(
...
if (n_class > 1 && !any(is.na(dataset_normal$class))) {
  ...
  diff_msg_med <- with(
    ...
    str_glue(
      "从学生的中位数水平来看，",
      "{group[which.max(med)]}班学生的整体表现最好，",
      "而{group[which.min(med)]}班学生的整体表现最差，",
      "{dinfer_msg_med}。"
    )
  )
}
...
```{r {{ab_code}}-boxplot-gender, fig.cap=str_glue("{{name_unit}}{{ab_name}}测评的性别差异对比"), fig.width=8, fig.asp=0.5}
list(
  school = dataset_normal,

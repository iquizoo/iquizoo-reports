...
```{r {{ab_code}}-histogram, fig.cap=str_glue("总体学生{{ab_name}}测评的分数分布"), fig.width=8, fig.asp=0.618}
dataset_hist <- list(
  全体 = dataset,
  本校 = dataset_unit
) %>%
...
stats_descr %>%
  knitr::kable(
    digits = 1,
    col.names = c(report_unit_name, "实测人数", "平均数", "标准差", "最高分", "中位数", "最低分"),
...
stats_level %>%
  ...
  knitr::kable(
    ...
    caption = str_glue("本次测评全体、本校及各{report_unit_name}学生{{ab_name}}测评的等级比例详表")
  )
```
...
diff_msg_med <- ""
if (n_grps > 1) {
  dinfer_med <- broom::tidy(kruskal.test(as.formula(str_glue("{{ab_code}} ~ {report_unit}")), dataset_unit))
  if (n_grps > 2) {

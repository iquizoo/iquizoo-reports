...
ggplot(dataset, aes({{ab_code}}, fill = group)) +
...
if (compare_unit != "school") {
  stats_descr_group <- dataset_with_level %>%
  ...
  stats_descr <- bind_rows(stats_descr_group, stats_descr_school) %>%
    mutate(
      group = factor(group, c(unique(stats_descr_group$group), "本校"))
    )
} else {
...
diff_msg_avg <- ""
if (n_grps > 1) {
  dinfer_avg <- broom::tidy(aov(as.formula(str_glue("{{ab_code}} ~ {compare_unit}")), dataset))
  if (n_grps > 2) {
...
# summary statistics of levels for whole and each report unit
stats_level_school <- dataset_with_level %>%
  ...
  add_column(group = "本校", .before = 1)
if (compare_unit != "school") {
  stats_level_group <- dataset_with_level %>%
    rename(group = !!compare_unit) %>%
    group_by(group) %>%
    mutate(cnt = sum(!is.na({{ab_code}}))) %>%
  ...
  stats_level <- bind_rows(stats_level_group, stats_level_school) %>%
    mutate(
      group = factor(group, c(unique(stats_level_group$group), "本校"))
    ) %>%
    complete(
      level, nesting(group, cnt), 
      fill = list(n_level = 0, prop_level = 0)
    )
} else {
...
diff_msg_med <- ""
dinfer_med <- broom::tidy(kruskal.test(as.formula(str_glue("{{ab_code}} ~ {compare_unit}")), dataset))
if (n_grps > 2) {

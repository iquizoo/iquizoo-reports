...
ggplot(dataset_unit, aes({{ab_code}})) +
  geom_density(color = NA, alpha = 0.5, fill = "lightblue") +
  geom_vline(
    xintercept = mean(dataset_unit${{ab_code}}, na.rm = TRUE), 
    color = "red"
  ) +
...
stats_descr_school <- dataset_unit %>%
  summarise(
...
if (compare_unit != "school") {
  stats_descr_group <- dataset_unit %>%
  ...
  stats_descr <- bind_rows(stats_descr_group, stats_descr_school) %>%
    mutate(group = factor(group) %>% fct_relevel("本校", after = Inf))
} else {
...
表\@ref(tab:{{ab_code}}-describe)中展示了本次测评全体、本校和各年级的学生整体表现情况。`r diff_msg_avg`
...
表\@ref(tab:{{ab_code}}-levels)给出了所测各学校学生的各水平人数。划分方式如图\@ref(fig:blai-levels-rule)。

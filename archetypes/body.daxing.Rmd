...
```{r {{ab_code}}-histogram, fig.cap=str_glue("{{customer_name}}总体学生{{ab_name}}测评的分数分布"), dev='Cairo_png', fig.width=12, fig.asp=0.618}
dataset %>%
  ggplot(aes({{ab_code}})) +
...
diff_avg <- broom::tidy(aov({{ab_code}} ~ school, dataset_normal))
...
本区共施测`r n_distinct(dataset$school)`所学校。表\@ref(tab:{{ab_code}}-describe)中展示了本区及各学校的学生整体表现情况。从学生的平均水平来看，`r with(stats_descr_school, group[avg == max(avg)])`的学生表现最好，而`r with(stats_descr_school, group[avg == min(avg)])`的学生表现最差，`r diff_msg_avg`。
...
stats_level %>%
  ...
  knitr::kable(
    ...
    caption = str_glue("{{customer_name}}及各学校学生{{ab_name}}测评的等级比例详表")
  )
```
...
```{r {{ab_code}}-boxplot-gender, fig.cap=str_glue("{{customer_name}}{{ab_name}}测评的性别差异对比"), fig.width=12, fig.asp=0.5, dev='Cairo_png'}
list(
...
本区学生的{{ab_name}}测评分数的性别对比如图\@ref(fig:{{ab_code}}-boxplot-gender)。从中位数（即得分恰好在正中间的学生）角度看，`r if_else(with(dataset_normal, median({{ab_code}}[gender == "男"]) > median({{ab_code}}[gender == "女"])), "男生的表现好于女生", "女生的表现好于男生")`，`r diff_msg_gender`。

```{r {{ab_code}}-boxplot-school, fig.cap=str_glue("{{customer_name}}及各学校学生{{ab_name}}测评的得分情况对比"), fig.width=12, fig.asp=1.2, dev='Cairo_png'}
...
diff_school <- broom::tidy(kruskal.test(with(dataset_normal, split({{ab_code}}, school))))
if (diff_school$p.value > 0.05) {
  diff_msg_school <- "但是各学校的表现差异不明显"
} else {
  diff_msg_school <- "并且各学校的表现差异明显"
}
```

本区及各个学校的{{ab_name}}测评分数的分布区间如图\@ref(fig:{{ab_code}}-boxplot-school)。从中位数（即得分恰好在正中间的学生，参考表\@ref(tab:{{ab_code}}-tab)查看具体数值）角度看，`r with(stats_descr_school, group[med == max(med)])`的表现最好，`r with(stats_descr_school, group[med == min(med)])`的表现最差，`r diff_msg_school`。在后期应该重点关注`r with(stats_descr_school, group[med == min(med)])`，在该校进行相关基础能力课程的推广，对该校能力不足的学生进行干预和有计划的训练。

```{r {{ab_code}}-correlation, fig.cap=str_glue("{{ab_name}}测评得分和数学成绩的关系"), fig.width=12, fig.asp=0.618, dev='Cairo_png'}
mathscore <- haven::read_sav("assets/extra/Math.sav", encoding = "gbk") %>%
  select(stu_code, total_score)
dataset_with_math <- dataset_normal %>%
  inner_join(mathscore, by = c("idNumber1" = "stu_code"))
ggplot(dataset_with_math, aes({{ab_code}}, total_score, group = 1)) +
  geom_point(alpha = 0.5) +
  stat_smooth(method = "glm", se = FALSE) +
  theme_classic(base_size = 18, base_family = text_family) +
  labs(x = "{{ab_name}}", y = "数学成绩")
corr_stats <- cor.test(~ {{ab_code}} + total_score, dataset_with_math)
```

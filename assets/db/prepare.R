...
for (base_dataset in base_datasets) {
  ...
  # load configurations
  dataset_configs <- config::merge(base_configs, config::get(config = base_dataset))
  data_dir <- file.path(dataset_configs$data_path, base_dataset)
  ...
  # load dataset ----
  data_origin <- switch(
    tools::file_ext(dataset_configs$data_file),
    xls = ,
    xlsx = read_excel(
      file.path(data_dir, dataset_configs$data_file),
      guess_max = 1048576 # maximal number of rows
    ),
    json = jsonlite::read_json(
      file.path(data_dir, dataset_configs$data_file), simplifyVector = TRUE
    ) %>%
      as.tbl() %>%
  ...
  if (!is.null(dataset_configs$user_correction)) {
    user_correction_varnames <- names(dataset_configs$user_correction$replace)
    for (user_correction_varname in user_correction_varnames) {
      data_origin[[user_correction_varname]] <-
        dataset_configs$user_correction$replace[[user_correction_varname]]
    }
  }
  # this is a special correction, currently only used for 'erxiao' dataset
  if (!is.null(dataset_configs$user_correction$special)) {
    data_origin <- switch(
      dataset_configs$user_correction$special,
  ...
  copy_to(iquizoo_db, users, "users_to_write", overwrite = TRUE)
  ...
  scores_corrected <- with(
    dataset_configs$score_correction,
    switch(
      type,
      none = data_origin %>%
  ...
  copy_to(iquizoo_db, scores, "scores_to_write", overwrite = TRUE)
  dbExecute(iquizoo_db, "BEGIN;")
  dbExecute(
    iquizoo_db, "
    REPLACE INTO scores
  ...
  if (!is.null(dataset_configs$ability_correction)) {
    ability_scores <- with(
      dataset_configs$ability_correction,
      switch(
        type,
        modify = {
  ...
  copy_to(iquizoo_db, ability_scores, "ab_scores_to_write", overwrite = TRUE)
  dbExecute(iquizoo_db, "BEGIN;")
  dbExecute(
    iquizoo_db, "
  ...
  if (file.exists(file.path(data_dir, dataset_configs$cover$class))) {
    class_covers <- read_excel(file.path(data_dir, dataset_configs$cover$class)) %>%
      select(one_of(key_vars[["class_cover"]]))
    copy_to(iquizoo_db, class_covers, "class_covers_to_write", overwrite = TRUE)
    dbExecute(iquizoo_db, "BEGIN;")
    dbExecute(
      iquizoo_db, "
  ...
  if (file.exists(file.path(data_dir, dataset_configs$cover$school))) {
    school_covers <- read_excel(file.path(data_dir, dataset_configs$cover$school)) %>%
      select(one_of(key_vars[["school_cover"]]))
    copy_to(iquizoo_db, school_covers, "school_covers_to_write", overwrite = TRUE)
    dbExecute(iquizoo_db, "BEGIN;")
    dbExecute(
      iquizoo_db, "
  ...
  if (file.exists(file.path(data_dir, dataset_configs$detail$school))) {
    school_details <- read_excel(file.path(data_dir, dataset_configs$detail$school)) %>%
      select(one_of(key_vars[["school_detail"]]))
    copy_to(iquizoo_db, school_details, "school_details_to_write", overwrite = TRUE)
    dbExecute(iquizoo_db, "BEGIN;")
    dbExecute(
      iquizoo_db, "

...
```{r <<ability_name>>-histogram, echo=FALSE, warning=FALSE, message=FALSE}
scores_this_ability %>%
  filter(region != "各班") %>%
...
```{r <<ability_name>>-boxplot, echo=FALSE, warning=FALSE, message=FALSE}
...
```{r <<ability_name>>-grading, echo=FALSE, warning=FALSE, message=FALSE}
ability_summary %>%
  gather(level, n, A:D) %>%
  mutate(
    prop = n / 实测人数 * 100,

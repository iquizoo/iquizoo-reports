...
**应测人数**：`r length(unique(user_info$userId))`人
...
fig_captions <- paste0(school, "学生学业成绩和BLAI指数的相关关系")
BLAI_exam <- user_exam %>%
  inner_join(BLAI) %>% 
  filter(!is.na(成绩), !is.na(BLAI))
BLAI_exam %>%
...
# set figure caption titles
fig_captions <- paste0(school, "学生学业成绩和", abilities, "指数的相关关系")
# plot figures for each ability
for (ability_name in abilities){
  ...
  ability_exam <- user_exam %>%
    ...
    filter(!is.na(成绩), !is.na(abscore))
  # if no data for this ability, continue to the next

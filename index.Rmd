...
# load ability scores
ability_scores_all <- read_excel(ability_scores_file)
# filter out scores for current school
if (!school_name %in% ability_scores_all$school) stop("School not found!")
ability_scores_school <- ability_scores_all %>%
  filter(school == school_name)
# gather school data and total data
ability_scores <- list(本区 = ability_scores_all, 本校 = ability_scores_school) %>%
  bind_rows(.id = "type")
# set test date
if (test_date_auto) {

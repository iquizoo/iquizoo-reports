...
library(readxl)
library(extrafont)
font_import(prompt = FALSE, pattern = "wqy")
text_family <- "WenQuanYi Zen Hei"
# do not display NA
...
ab_type <- "学习能力指数"
dataset_combined <- list(本区 = ability_scores_district, 本校 = ability_scores_school, 各班 = ability_scores_school) %>%
  bind_rows(.id = "type") %>%
  filter(ability == ab_type) %>%
  mutate(cls = if_else(type != "各班", NA_character_, cls))
bp_stats <- boxplot.stats(dataset_combined$score)
```
...
ggplot(dataset_combined, aes(type, score, fill = cls)) +

...
```{r summary statistics for BLAI by grade, echo=FALSE}
BLAI %>%
  group_by(学校, 年级) %>%
  summarise(
    人数 = n(),
...
```{r summary statistics for BLAI by class, echo=FALSE}
BLAI %>%
  group_by(学校, 年级, 班) %>%
  summarise(
    人数 = n(),
...
```{r lesson and BLAI, echo=FALSE, fig.cap=fig_captions, message=FALSE, warning=FALSE}
...
```{r ATTENTION header, echo=FALSE, results='asis', eval=ability_name %in% abilities, warning=FALSE, message=FALSE}
# header message
cat("
## 注意力指数

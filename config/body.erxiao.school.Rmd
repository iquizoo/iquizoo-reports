...
scores_this_ability %>%
  ...
  geom_vline(xintercept = with(ability_summary, 平均分[region != "各班"]), color = "red") +
...
scores_this_ability %>%
  mutate(
    cls = factor(
      cls, levels = rev(c("本校", ability_summary_cls_sortmed$cls))
    )
  ) %>%
  ...
  set_theme_panels(base_size = 10, base_family = text_family)
```
...
ability_summary %>%
  ...
  mutate(
    cls = factor(
      cls, levels = rev(c("全国平均", "本校", ability_summary_cls_sortmed$cls))
    )
  ) %>%
  gather(level, n, A:D) %>%
  mutate(
    prop = n / 实测人数 * 100,
    n_label = if_else(n == 0, NA_real_, n),
    prop_label = if_else(round(prop) == 0, NA_real_, round(prop))
  ) %>%
  ggplot(aes(cls, prop, fill = level, label = n_label)) +
